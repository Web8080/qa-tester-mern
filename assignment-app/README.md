...
# If you see "command not found: docker-compose", use "docker compose" (space) instead.

# Run smoke tests
docker compose exec api npm run test:smoke
```

On Docker Desktop (Compose V2), use `docker compose` (with a space) instead of `docker-compose`.
...
# Run E2E tests (Playwright; app and API must be running)
npx playwright install chromium   # once
npm run test:e2e
# If frontend is on port 3002 (e.g. Docker): PLAYWRIGHT_BASE_URL=http://localhost:3002 npm run test:e2e
```
...
Use `docker compose` (space) on Docker Desktop. Legacy installs may use `docker-compose` (hyphen).
